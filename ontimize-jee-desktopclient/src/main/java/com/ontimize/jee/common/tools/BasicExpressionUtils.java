...
public final class BasicExpressionUtils {
	 ...
	 */
	public static boolean areEquals(final BasicExpression newExpr, final BasicExpression oldExpr) {
		if (((newExpr == null) && (oldExpr != null)) || ((newExpr != null) && (oldExpr == null))) {
			return false;
		} else if (newExpr == null) {
			return true;
		}
		final Object[] newOperands = new Object[] { newExpr.getLeftOperand(), newExpr.getRightOperand() };
		final Object[] oldOperands = new Object[] { oldExpr.getLeftOperand(), oldExpr.getRightOperand() };
		if (!newExpr.getOperator().equals(oldExpr.getOperator())) {
			return false;
		}
		for (int i = 0; i < newOperands.length; i++) {
			final Object nop = newOperands[i];
			final Object oop = oldOperands[i];
	 ...
	 */
	public static BasicExpression combineExpression(final BasicExpression... exprs) {
	 ...
	 * @return the basic expression
	 */
	public static BasicExpression combineExpressionOr(final BasicExpression... exprs) {
	 ...
	 * @return
	 */
	public static BasicExpression combineExpression(final Operator op, final BasicExpression... exprs) {
		final ArrayList<BasicExpression> join = new ArrayList<>();
		if (exprs != null) {
			for (final BasicExpression expr : exprs) {
				if (expr != null) {
					join.add(expr);
				}
			}
		...
		for (final BasicExpression newExpr : join) {
			if (fullExpr == null) {
				fullExpr = newExpr;
			} else {
	 ...
	 * @return the basic expression
	 */
	public static BasicExpression convertSearchValue(final BasicField basicField, final SearchValue searchValue) {
	 ...
	 * @return
	 */
	private static BasicExpression convertSearchValueInComplex(final BasicField basicField, final List<Object> value) {
	...
	private static BasicExpression convertSearchValueNotBetween(final BasicField basicField, final SearchValue searchValue) {
	...
	private static BasicExpression convertSearchValueBetween(final BasicField basicField, final SearchValue searchValue) {
		return new BasicExpression(
	 ...
	 * @return the basic expression
	 */
	public static BasicExpression completeExpresionFromKeys(final BasicExpression sourceExpr, final Form form,
			final List<Object> otherKeys) {
		final Map<Object, Object> kv = new HashMap<>();
		for (final Object obj : otherKeys) {
			if (obj instanceof String) {
				// Direct filter from attr in form to db column
				final Object value = form.getDataFieldValue((String) obj);
				if (value != null) {
					kv.put(obj, value);
				}
			} else if (obj instanceof SimpleFieldFilter) {
				// Simple filter from attr in form to another named db column
				final Object value = form.getDataFieldValue(((SimpleFieldFilter) obj).getFormFieldAttr());
				if (value != null) {
	...
	public static BasicExpression completeExpresionFromKeys(BasicExpression sourceExpr, final Map<Object, Object> kv,
			final List<Object> otherKeys) {

		for (final Object obj : otherKeys) {
			if (obj instanceof String) {
				// Direct filter from attr in form to db column
				final Object value = kv.get(obj);
				if (value != null) {
					final BasicField basicField = new BasicField((String) obj);
					final BasicExpression fieldExpr = BasicExpressionUtils.composeExpressionFromValue(basicField, value);
				...
				if (value != null) {
					final BasicField basicField = new BasicField(((SimpleFieldFilter) obj).getDbAttr());
				...
				// Simple filter from two attrs in form (since - to) to single db column
				final Object fromDate = BasicExpressionUtils
						.getDate(kv.get(((BetweenDateFilter) obj).getSinceFormFieldAttr()));
				final Object toDate = BasicExpressionUtils.getDate(kv.get(((BetweenDateFilter) obj).getToFormFieldAttr()));
				BasicExpression fieldExpr = null;
				final BasicField basicField = new BasicField(((BetweenDateFilter) obj).getDbAttr());
				...
				if (fieldExpr != null) {
					...
					final Object dataFieldValue = kv.get(((BetweenDateFilter) obj).getCheckFieldAttr());
	 ...
	 * @return the basic expression
	 */
	public static BasicExpression composeExpressionFromValue(final BasicField basicField, final Object value) {
	 ...
	 * @return the basic expression
	 */
	public static BasicExpression findConditionForFields(final BasicExpression expr, final String... fieldNames) {
		if ((expr == null) || (fieldNames == null) || (fieldNames.length == 0)) {
			return null;
		}
		final List<BasicExpression> validExpr = new ArrayList<>();
		final Set<String> fields = new HashSet<>(Arrays.asList(fieldNames));
	 ...
	 */
	private static void findConditionForFields(final BasicExpression expr, final List<BasicExpression> validExpr,
			final Set<String> fields) {
		final Object leftOperand = expr.getLeftOperand();
		final Object rightOperand = expr.getRightOperand();
		final Object[] toCheck = new Object[] { leftOperand, rightOperand };
		for (final Object check : toCheck) {
	 ...
	 * @return the basic expression
	 */
	public static BasicExpression copyWithoutFields(final BasicExpression expr, final String... fieldNames) {
		if (expr == null) {
			return null;
		}
	 ...
	 * @return
	 */
	public static BasicExpression replaceExpression(final BasicExpression originalExpr, final BasicExpression ipExpr,
			final BasicExpression replaceExpr) {
		if (originalExpr == ipExpr) {
			return replaceExpr;
		}
		final Object leftOperand = originalExpr.getLeftOperand();
		Object newLeft = null;
		if (leftOperand instanceof BasicExpression) {
		...
		final Object rightOperand = originalExpr.getRightOperand();
		Object newRight = null;
		if (rightOperand instanceof BasicExpression) {
	 ...
	 */
	public static void renameField(final BasicExpression expr, final String fieldName, final String newFieldName) {
	 ...
	 * @return true, if successful
	 */
	public static boolean containsField(final BasicExpression expr, final String fieldName) {
	...
	public static class SimpleFieldFilter {
		 ...
		 */
		public SimpleFieldFilter(final String formFieldAttr, final String dbAttr) {
		 ...
		 */
		public SimpleFieldFilter(final String formFieldAttr, final String dbAttr, final boolean useContains) {
	...
	public static class BetweenDateFilter {
		 ...
		 * @param dbAttr the db attr
		 */
		public BetweenDateFilter(final String sinceFormFieldAttr, final String toFormFieldAttr, final String dbAttr) {
		 ...
		 * @param dbAttr the db attr
		 */
		public BetweenDateFilter(final String sinceFormFieldAttr, final String toFormFieldAttr, final String checkFieldAttr,
				final String dbAttr) {
	...
	public static String toString(final BasicExpression be) {
		final StringBuilder sb = new StringBuilder();
		BasicExpressionUtils.toString(be, sb);
		return sb.toString();
	}

	private static void toString(final BasicExpression be, final StringBuilder sb) {
		if (be == null) {
			return;
		}
		sb.append("(");
		final Object left = be.getLeftOperand();
		...
		final Object right = be.getRightOperand();
		if (right != null) {
