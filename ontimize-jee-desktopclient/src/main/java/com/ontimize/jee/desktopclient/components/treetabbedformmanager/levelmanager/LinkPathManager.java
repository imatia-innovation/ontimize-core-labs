...
public class LinkPathManager implements PathManager {
	...
	public LinkPathManager(final Map<Object, Object> parameters) {
		this.init(parameters);
	}

	private void init(final Map<Object, Object> parameters) {
	...
	@Override
	public void showPath(final List<Level> pathLevels) {
		...
		int i = 0;
		for (final Level level : LinkPathManager.this.pathLevels) {
		...
		for (final Row r : LinkPathManager.this.rows) {
			r.updateUI();
		}
	...
	@Override
	public void setLevelManager(final LevelManager levelManager) {
	...
	public Component createGUIComponent() {
		final Map<Object, Object> params = new Hashtable<>();
	...
	private static void addLevel(final Column column, final List<Row> rows, final Level level,
			final LinkPathManager linkPathManager, final String separator, final boolean isLast,
		...
		if (rows.size() == 0) {
			...
			final Row current = rows.get(rows.size() - 1);
		...
		if ((rows.size() > 1) || (r.getComponentCount() > 0)) {
			final JLabel l = new JLabel(separator);
			l.setOpaque(false);
			r.add(l);
		}
		r.add(levelButton);
	...
	private static String translate(final String text, final ResourceBundle resourceBundle) {
		...
		protected void fireActionPerformed(final ActionEvent evt) {
			...
			for (int i = 0; i < listeners.length; i += 2) {
				if (listeners[i] == ActionListener.class) {
					final ActionListener listener = (ActionListener) listeners[i + 1];
