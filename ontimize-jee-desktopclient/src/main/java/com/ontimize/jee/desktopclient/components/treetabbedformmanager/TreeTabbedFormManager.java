...
public class TreeTabbedFormManager extends BaseFormManager implements ITreeTabbedFormManager {
	 ...
	 * </Table>
	 */
	public TreeTabbedFormManager(final Map parameters) throws Exception {
		super(parameters);
	}
	...
	public void init(final Map parameters) throws Exception {
		...
		final Object oTreeBuilder = parameters.get(ITreeTabbedFormManager.TREE_TABLE_BUILDER);
		...
		final Object tree = parameters.get(ITreeTabbedFormManager.TREE);
			...
			if (!this.isLoaded()) {
				...
				try {
					...
					if (this.treeFileName != null) {
						try {
							final Form mockForm = new Form(new Hashtable<>());
	...
	public TreeTabbedFormManager(final Frame frame, final Container container, final FormBuilder builder, final String formURI,
	...
	private void createLayoutWithTree(final Frame frame, final Container container, final FormBuilder builder, final String formURI,
			...
			final LevelManager treeTableColumn, final boolean controls) {
		...
		if (SwingUtilities.isEventDispatchThread()) {
			...
			try {
				SwingUtilities.invokeAndWait(new Runnable() {
					...
					@Override
					public void run() {
						final boolean bIn = TreeTabbedFormManager.this.showFormInEDTh(formName);
						if (bIn) {
	...
	@Override
	public int indexOf(final TreeTabbedDetailForm searchedForm) {
		final Map keyValues = searchedForm.getKeysValues();
		final String entityName = searchedForm.getEntityName();
		...
		for (int i = 1; i < size; i++) {
			...
			if (component instanceof TreeTabbedDetailForm) {
				final TreeTabbedDetailForm detailForm = (TreeTabbedDetailForm) component;
						...
						for (final Object key : keyAttrs) {
							if (keyValues.containsKey(key)) {
								...
								if ((keyValue instanceof List) && (((List) keyValue).size() > 0)) {
									keyValue = CollectionTools.firstElement((List) keyValues.get(key));
								}
	...
	protected static class FormTabbedPane extends JTabbedPane {
		...
		public void replaceTab(final int index, final String title, final Component component) {
