		...
		if (this.bRequest != null) {
			this.bRequest.addActionListener(new ActionListener() {
				...
				public void actionPerformed(final ActionEvent e) {
					final int selectedRow = IMRequest.this.tResults.getSelectedRow();
					if (selectedRow != -1) {
						final Map selectedRowData = IMRequest.this.tResults.getSelectedRowData();
						final String serviceName = ((List<String>) selectedRowData.get("SERVICE_NAME")).get(0);
						final String methodName = ((List<String>) selectedRowData.get("METHOD_NAME")).get(0);
