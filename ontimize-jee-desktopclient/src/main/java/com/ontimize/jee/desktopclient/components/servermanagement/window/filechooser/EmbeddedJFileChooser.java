...
public class EmbeddedJFileChooser extends JPanel {
	...
	public EmbeddedJFileChooser(final MODE mode, final IEmbeddedJFileChooser listener) {
		super();
		this.mode = mode;
	...
	public void setFileFilter(final FileNameExtensionFilter fileNameExtensionFilter) {
	...
	private Component createRowButtonsComp(final Button... buttons) {
		final Map<Object, Object> parameters = new Hashtable<>();
		parameters.clear();
		final Row row = new Row(parameters);
		...
		final Column column = new Column(parameters);

		column.add(row);

		parameters.clear();
		parameters.put("expand", "yes");
		final Column column2 = new Column(parameters);

		parameters.clear();
		parameters.put("expand", "no");
		final Row row2 = new Row(parameters);
	...
	public class OpenFileEmbbededListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			new Thread(new Runnable() {
				...
				@Override
				public void run() {
					final File selectedFile = EmbeddedJFileChooser.this.fileChooser.getSelectedFile();
	...
	public class SaveFileEmbbededListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			new Thread(new Runnable() {
				...
				@Override
				public void run() {
					final File currentDirectory = EmbeddedJFileChooser.this.fileChooser.getCurrentDirectory();
					final File selectedFile = EmbeddedJFileChooser.this.fileChooser.getSelectedFile();
					final String fileName = (String) EmbeddedJFileChooser.this.fileName.getValue();
					...
					if (fileName == null) {
						...
						if (selectedFile != null) {
							if (selectedFile.getName().equals(fileName)) {
								final boolean showQuestionMessage = MessageDialog
	...
	public class CustomJFileChooser extends JFileChooser {
		...
		private void disableTextComponents(final Container parent) {
			final Component[] c = parent.getComponents();
			for (int j = 0; j < c.length; j++) {
				final String unpack = this.unpack(c[j]);
		...
		private String unpack(final Component c) {
