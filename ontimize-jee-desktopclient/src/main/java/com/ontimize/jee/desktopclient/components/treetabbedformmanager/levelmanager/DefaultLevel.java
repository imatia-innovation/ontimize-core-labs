...
public class DefaultLevel extends Table implements Level {
	...
	public DefaultLevel(final Map params) throws Exception {
		super(params);
		...
		this.getJTable().addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(final MouseEvent e) {
				final IFormManager formManager = DefaultLevel.this.getParentForm().getFormManager();
	...
	public Map getParentKeyValues(final boolean applyEquivalences) {
		final Level level = this.levelManager.getLevel(this.previousLevelId);

		final Map<String, List<?>> parentSelectedRowData = level != null ? level.getSelectedData() : null;
		final Map kv = new Hashtable();
		if ((this.parentkeys != null) && (parentSelectedRowData != null)) {
			for (int i = 0; i < this.parentkeys.size(); i++) {
				final List<?> v = parentSelectedRowData.get(this.parentkeys.get(i).toString());
				if ((v != null) && (v.size() == 1)) {
	...
	public void init(final Map parameters) throws Exception {
		...
		if (this.displayTextFormat != null) {
			...
			final String oDateFormat = ParseUtils.getString((String) parameters.get(Level.DISPLAY_TEXT_DATE_FORMAT), null);
			if (oDateFormat != null) {
	...
	@Override
	public void setNextLevelId(final String linkedTable) {
	...
	@Override
	public void setPreviousLevelId(final String previousLevelId) {
	...
	public void openFormInMainTab(final int rowIndex) {
	...
	public Map<?, ?> getKeysValues() {
		final Map<Object, Object> keysValues = new HashMap<>();
		final List<?> keys2 = this.getKeys();
		final Map<String, List<?>> selectedData = this.getSelectedData();

		if ((keys2 != null) && (selectedData != null)) {
			for (final Object o : keys2) {
				final List<?> list = selectedData.get(o);
	...
	private void updateLastSelected(final int selectedRow) {
		// Return a new hastable with the data
		final int[] selectedRows = { selectedRow };
	...
	protected void setInnerValue(final Object value, final boolean autoSizeColumns) {
		...
		if (!this.lastSelectedKeys.isEmpty()) {
			final int rowForKeys = this.getRowForKeys(new Hashtable<>(this.lastSelectedKeys));
			if (rowForKeys != -1) {
