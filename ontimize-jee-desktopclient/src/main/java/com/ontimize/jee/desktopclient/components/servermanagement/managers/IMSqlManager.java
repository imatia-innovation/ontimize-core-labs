...
public class IMSqlManager extends BasicInteractionManager {
		...
		this.sqlList.addListMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(final MouseEvent e) {
				if (e.getClickCount() == 2) {
					final SentenceSql sentence = (SentenceSql) IMSqlManager.this.sqlList.getSelectedEntry();
	...
	protected void loadHistoryList() {
		final MainApplication application = (MainApplication) ApplicationManager.getApplication();
		if (this.user == null) {
		...
		for (int i = 0; i < 1000; i++) {
			final String preference = application.getPreferences().getPreference(this.user, "SQLSentence" + i);
			if (preference != null) {
		...
		public void actionPerformed(final ActionEvent e) {
			new Thread(new Runnable() {
				...
				public void run() {
					...
					try {
						final List<String> availableDataSources = BeansFactory.getBean(IServerManagementService.class)
	...
	public class QueryButtonListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			new Thread(new Runnable() {
				...
				@Override
				public void run() {
					final String sql = (String) IMSqlManager.this.queryField.getValue();
					final String connectionName = (String) IMSqlManager.this.comboConnectionName.getValue();
					...
					if (sql != null) {
						try {
							...
							final Map<String, String> data = new Hashtable<>();
		 ...
		 * @param res the res
		 */
		public void printEntityResult(final EntityResult res) {
			switch (res.getCode()) {
		...
		public void actionPerformed(final ActionEvent e) {
			new Thread(new Runnable() {
				...
				public void run() {
					if ((IMSqlManager.this.queryField.getValue() != null)
							&& !"".equals(IMSqlManager.this.queryField.getValue())) {
						final SentenceSql sentence = new SentenceSql((String) IMSqlManager.this.queryField.getValue(),
								IMSqlManager.this.user);
	...
	public class DeletryEntry implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			new Thread(new Runnable() {
				...
				@Override
				public void run() {
					final Object selectedEntry = IMSqlManager.this.sqlList.getSelectedEntry();
		...
		public void actionPerformed(final ActionEvent e) {
			new Thread(new Runnable() {
				...
				public void run() {
					...
					for (int i = 0; i < 1000; i++) {
						final String preference = application.getPreferences()
	...
	public static class SentenceListRenderer extends JLabel implements ListCellRenderer {
		...
		public Component getListCellRendererComponent(final JList list, final Object value, final int index, final boolean isSelected,
			...
			final SentenceSql entry = (SentenceSql) value;
