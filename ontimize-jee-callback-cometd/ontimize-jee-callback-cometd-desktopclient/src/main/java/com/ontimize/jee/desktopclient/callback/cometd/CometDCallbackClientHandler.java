...
public class CometDCallbackClientHandler implements ICallbackClientHandler, InitializingBean {
	 ...
	 * @param url the new callback relative url
	 */
	public void setCallbackRelativeUrl(final String url) {
	 ...
	 * @param bayeuxClient the new bayeux client
	 */
	public void setBayeuxClient(final BayeuxClient bayeuxClient) {
	 ...
	 *
	 * @see
	 * com.ontimize.jee.desktopclient.callback.ICallbackClientHandler#addWebSocketEventListener(com.
	 * ontimize.jee.desktopclient.callback.ICallbackEventListener)
	 */
	@Override
	 ...
	 *
	 * @see
	 * com.ontimize.jee.desktopclient.callback.ICallbackClientHandler#removeWebSocketEventListener(com.
	 * ontimize.jee.desktopclient.callback.ICallbackEventListener)
	 */
	@Override
	...
	protected void connectionEstablished() {
		...
		final ClientSessionChannel ojeeCallbackChannel = this.bayeuxClient
				.getChannel(CometDCallbackConstants.ONTIMIZE_JEE_CALLBACK_CHANNEL);
		this.getBayeuxClient()
		.batch(() -> ojeeCallbackChannel.subscribe(CometDCallbackClientHandler.this.ojeeMessageListener,
				new MessageListener() {
			@Override
			public void onMessage(final ClientSessionChannel channel, final Message message) {
				CometDCallbackClientHandler.logger
				.info("On subscribe message from connectionEstablished: {}", message);
			}
		}));
	 ...
	 *
	 * @see
	 * com.ontimize.jee.desktopclient.callback.ICallbackClientHandler#sendMessage(java.lang.Integer,
	 * java.lang.Object)
	 */
	@Override
	...
	public void sendMessage(final Integer messageType, final String messageSubtype, final Object ob) throws OntimizeJEEException {
		...
		data.put(CometDCallbackConstants.KEY_DATA,
	...
	/**
	 * Fire message event.
	 * @param wrappedMessage the wrapped message
	 */
	protected void fireMessageEvent(final CallbackWrapperMessage wrappedMessage) {
	 ...
	 * is registered with a component using the component's <code>addConnectionListener<code> method.
	 * When the connection event occurs, that object's appropriate method is invoked.
	 *
	 * @see ConnectionEvent
	 */
	protected class ConnectionListener implements ClientSessionChannel.MessageListener {
		 ...
		 *
		 * @see
		 * org.cometd.bayeux.client.ClientSessionChannel.MessageListener#onMessage(org.cometd.bayeux.client.
		 * ClientSessionChannel, org.cometd.bayeux.Message)
		 */
		@Override
		public void onMessage(final ClientSessionChannel channel, final Message message) {
	 ...
	 * message event implements this interface, and the object created with that class is registered
	 * with a component using the component's <code>addMessageListener<code> method. When the message
	...
	protected class MessageListener implements ClientSessionChannel.MessageListener {
		...
		public void onMessage(final ClientSessionChannel channel, final Message message) {
			try {
				...
				final CallbackWrapperMessage wrappedMessage = this.deserialize((String) message.getData());
		...
		public CallbackWrapperMessage deserialize(final String message) {
			try {
				final String newMessage = new String(java.util.Base64.getDecoder().decode(message),
	...
	private static class OJettyCookieStore implements CookieStore {
		...
		public List<HttpCookie> getCookies() {
			...
			final List<HttpCookie> res = new ArrayList<>();

			// 1º- Add hessian cookies (distinct cookieName, not equals, for instance ignoring domain)
			final List<Cookie> hessianCookies = OntimizeHessianHttpClientSessionProcessorFactory.getCookieStore()
					.getCookies();
			for (final Cookie cookie : hessianCookies) {
		...
		private boolean checkCookieExists(final List<HttpCookie> cookieList, final Cookie cookieToSearch) {
			if (cookieList != null) {
		...
		private boolean checkCookieExists(final List<HttpCookie> cookieList, final HttpCookie cookieToSearch) {
			if (cookieList != null) {
				for (final HttpCookie cookie : cookieList) {
					if (ObjectTools.safeIsEquals(cookieToSearch.getName(), cookie.getName())) {
						return true;
