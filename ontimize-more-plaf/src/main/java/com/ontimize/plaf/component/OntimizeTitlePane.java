...
import com.ontimize.plaf.utils.OSwingUtilities;
...
public class OntimizeTitlePane extends JComponent {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (theTitle != null) {
            FontMetrics fm = OSwingUtilities.getFontMetrics(g);
            ...
            if (leftToRight) {
                ...
                theTitle = OSwingUtilities.clipStringIfNecessary(fm, theTitle, titleW);
            } else {
    ...
    protected void updateSystemIcon() {
        ...
        if (icons.size() == 0) {
            ...
            // Select the icon closest to the desired size
            Image selectedIcon = icons.get(0);
            int closestDiff = Math.abs(selectedIcon.getWidth(null) - OntimizeTitlePane.IMAGE_WIDTH);
            
            for (Image icon : icons) {
                int diff = Math.abs(icon.getWidth(null) - OntimizeTitlePane.IMAGE_WIDTH);
                if (diff < closestDiff) {
                    closestDiff = diff;
                    selectedIcon = icon;
                }
            ...
            // Scale if necessary
            if (selectedIcon.getWidth(null) != OntimizeTitlePane.IMAGE_WIDTH || 
                selectedIcon.getHeight(null) != OntimizeTitlePane.IMAGE_HEIGHT) {
                this.systemIcon = selectedIcon.getScaledInstance(OntimizeTitlePane.IMAGE_WIDTH,
                        OntimizeTitlePane.IMAGE_HEIGHT, Image.SCALE_SMOOTH);
            } else {
                this.systemIcon = selectedIcon;
            }
        }
    }
