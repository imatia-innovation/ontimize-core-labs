...
import com.ontimize.plaf.painter.util.CustomPaint9Painter;
...
public abstract class ImagePainter extends AbstractPainter {
    ...
    protected CustomPaint9Painter imageCache;
    ...
    protected static CustomPaint9Painter getPaint9Painter() {
        ...
        // we use a AppContext specific CustomPaint9Painter. It's backed via
        // a WeakRef so that it can go away if the look and feel changes.
        synchronized (ImagePainter.CACHE_KEY) {
            @SuppressWarnings("unchecked")
            WeakReference<CustomPaint9Painter> cacheRef = (WeakReference<CustomPaint9Painter>) OAppContext
                .get(ImagePainter.CACHE_KEY);
            CustomPaint9Painter painter;
            if ((cacheRef == null) || ((painter = cacheRef.get()) == null)) {
                painter = new CustomPaint9Painter(30);
                cacheRef = new WeakReference<>(painter);
                OAppContext.put(ImagePainter.CACHE_KEY, cacheRef);
            }
            return painter;
        }
    }
    ...
    protected void paint(JComponent component, Graphics g, int x, int y, int w, int h) {
        ...
        if (CustomPaint9Painter.validImage(image)) {
            CustomPaint9Painter.PaintType type;
            if (this.getCenter()) {
                type = CustomPaint9Painter.PaintType.CENTER;
            } else if (!this.getTiles()) {
                type = CustomPaint9Painter.PaintType.PAINT9_STRETCH;
            } else {
                type = CustomPaint9Painter.PaintType.PAINT9_TILE;
            }
            int mask = CustomPaint9Painter.PAINT_ALL;
            if (!this.getCenter() && !this.getPaintsCenter()) {
                mask |= CustomPaint9Painter.PAINT_CENTER;
            }
