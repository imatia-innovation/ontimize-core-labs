...
public abstract class CachedPainter {
    ...
    /**
     * ImageCache is used to cache images based on a set of arguments.
     */
    private static class ImageCache {
        private static final int MAX_CACHED_SIZE = 5 * 8;
        private final LinkedHashMap<String, Image> cache;
        private final int maxCount;

        ImageCache(int maxCount) {
            this.maxCount = maxCount;
            this.cache = new LinkedHashMap<String, Image>(maxCount, 0.75f, true) {
                @Override
                protected boolean removeEldestEntry(Map.Entry<String, Image> eldest) {
                    return size() > ImageCache.this.maxCount;
                }
            };
        }
        ...
        synchronized Image getImage(Object key, GraphicsConfiguration config, int w, int h, Object[] args) {
            String hashKey = getHashKey(key, config, w, h, args);
            return cache.get(hashKey);
        }

        synchronized void setImage(Object key, GraphicsConfiguration config, int w, int h, Object[] args,
                Image image) {
            String hashKey = getHashKey(key, config, w, h, args);
            cache.put(hashKey, image);
        }
        ...
        private String getHashKey(Object key, GraphicsConfiguration config, int w, int h, Object[] args) {
            StringBuilder sb = new StringBuilder();
            sb.append(key).append("_").append(config).append("_").append(w).append("x").append(h);
