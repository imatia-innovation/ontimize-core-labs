...
public class UserManagementKeycloakImpl implements IUserManagement {
	...
	private static final ResteasyClient					CONNECTION_POOL		= new ResteasyClientBuilderImpl()
	...
	public List<RealmInfo> getRealmsForUser(final String username) {
		...
		for (final RealmRepresentation r : rreps) {
	...
	public RealmResource createRealmIfNotExists(final String realm, final String displayName, final String... clients) {
		...
		try (Response resp = rr.clients().create(cr)) {
			...
			for (final String client : clients) {
				...
				final List<String> redirectUris = new ArrayList<>(Arrays.asList(clients));
	...
	public GroupRepresentation getGroup(final String groupName) {
		...
		final Optional<GroupRepresentation> group = groupList.stream().filter(g -> groupName.equals(g.getName())).findAny();
	...
	public void addRealmToUser(final String username, final String realm) {
		...
		final List<UserRepresentation> userList = this.searchUserAccount(username);
		...
		if (userList != null && !userList.isEmpty()) {
			...
			final Map<String, List<String>> attributes = userRep.getAttributes();

			if (attributes == null) {
				userRep.singleAttribute(TENANTS_KEY, realm);
			} else if (attributes.containsKey(TENANTS_KEY)) {
				final List<String> attr = attributes.get(TENANTS_KEY);
				attr.add(realm);
	...
	public void setRolesForUser(final String realm, final String client, final List<UserRoles> userRoles) {
		...
		for (final UserRepresentation userRep : userR) {
	...
	public void setApplicationRolesForUser(final String realm, final String user,
		...
		if (ur.isPresent()) {
			...
			for (final ApplicationRoles appRole : appRoles) {
	...
	private ClientResource searchClient(final String name, final RealmResource realm) {
	...
	private RealmResource searchRealm(final String realm) {
		return this.searchRealm(realm, this.getInstance());
	}

	private RealmResource searchRealm(final String realmName, final Keycloak keycloak) {
		final List<RealmRepresentation> rreps = keycloak.realms().findAll();
