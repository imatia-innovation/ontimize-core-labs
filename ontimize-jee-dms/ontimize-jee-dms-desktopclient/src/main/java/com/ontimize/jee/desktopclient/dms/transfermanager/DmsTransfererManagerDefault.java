...
public class DmsTransfererManagerDefault implements Observer, IDmsTransfererManager {
	...
	public void registerTransferer(final Class<AbstractDmsTransferable> transferable, final AbstractDmsTransferer<AbstractDmsTransferable> transferer) {
		this.transferersFactory.put(transferable, transferer);
	}

	protected <T extends AbstractDmsTransferable> AbstractDmsTransferer<T> getTransfererForTransferable(final Class<T> clazz) throws DmsException {
		final AbstractDmsTransferer<T> transferer = (AbstractDmsTransferer<T>) this.transferersFactory.get(clazz);
		if (transferer == null) {
	...
	public void setSyncFolder(final Path syncFolder) {
	 ...
	 */
	@Override
	public void transfer(final AbstractDmsTransferable transferable) throws DmsException {
	...
	public void update(final Observable observable, final Object arg) {
		...
		final AbstractDmsTransferable transfer = (AbstractDmsTransferable) observable;
		final Status state = transfer.getStatus();
	...
	@Override
	public Path obtainDmsFileVersion(final Serializable idFileVersion) throws DmsException {
		final EntityResult er = BeansFactory.getBean(IDMSService.class).fileVersionQuery(idFileVersion,
		        Arrays.asList(new String[] { DMSNaming.DOCUMENT_FILE_VERSION_FILE_SIZE, DMSNaming.DOCUMENT_FILE_NAME }));
		final Long fileSize = ((List<Number>) er.get(DMSNaming.DOCUMENT_FILE_VERSION_FILE_SIZE)).get(0).longValue();
		final String fileName = ((List<String>) er.get(DMSNaming.DOCUMENT_FILE_NAME)).get(0);
		return this.obtainDmsFileVersion(idFileVersion, fileName, fileSize);
	}

	@Override
	public Path obtainDmsFileVersion(final Serializable idFileVersion, final String fileName, final Long fileSize) throws DmsException {
		final Path localFile = this.computeLocalFile(idFileVersion, fileName);
	...
	private Path computeLocalFile(final Object idFileVersion, final String filename) throws DmsException {
	...
	@Override
	public synchronized void addTransferQueueListener(final ITransferQueueListener transferQueueListener) {
		this.transferQueueListeners.add(transferQueueListener);
	}

	@Override
	public synchronized void removeTransferQueueListener(final ITransferQueueListener transferQueueListener) {
		this.transferQueueListeners.remove(transferQueueListener);
	}

	public synchronized void fireTransferQueueChangedEvent(final TransferQueueChangedEvent evt) {
		for (final ITransferQueueListener listener : this.transferQueueListeners) {
	...
	protected class TransferJob<T extends AbstractDmsTransferable> implements Runnable {
		...
		public TransferJob(final T transferable) {
			super();
		...
		public void run() {
			try {
				final AbstractDmsTransferer<T> transferer = (AbstractDmsTransferer<T>) DmsTransfererManagerDefault.this.getTransfererForTransferable(this.transferable.getClass());
