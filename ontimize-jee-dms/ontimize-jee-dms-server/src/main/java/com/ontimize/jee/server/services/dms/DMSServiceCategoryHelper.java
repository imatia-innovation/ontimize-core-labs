...
public class DMSServiceCategoryHelper extends AbstractDMSServiceHelper {
	 ...
	 */
	public DMSCategory categoryGetForDocument(final Serializable idDocument, final List<?> attributes) {
		...
		final Map<Object, Object> filter = EntityResultTools.keysvalues(this.getColumnHelper().getDocumentIdColumn(),
				idDocument);
		final EntityResult er = this.daoHelper.query(this.categoryDao, filter, attribs);
	 ...
	 * @param av the av
	 */
	public void categoryUpdate(final Serializable idCategory, Map<?, ?> av) {
	 ...
	 * @param idCategory the id category
	 */
	public void categoryDelete(final Serializable idCategory) {
		CheckingTools.failIfNull(idCategory, DMSNaming.ERROR_DOCUMENT_ID_MANDATORY);
		final Map<String, Object> filter = new HashMap<>();
		filter.put(this.getColumnHelper().getCategoryIdColumn(), idCategory);
		...
		avUpdate.put(this.getColumnHelper().getCategoryParentColumn(),
				new NullValue(this.getSQLTypeFromClass(idCategory.getClass())));
		...
		} catch (final SQLWarningException ex) {
	...
	protected int getSQLTypeFromClass(final Class valueClass) {
	 ...
	 * @return the object
	 */
	public Serializable categoryInsert(final Serializable idDocument, final String name, final Serializable idParentCategory,
			final Map<?, ?> otherData) {
		CheckingTools.failIfNull(idDocument, DMSNaming.ERROR_DOCUMENT_ID_MANDATORY);
		CheckingTools.failIfNull(name, DMSNaming.ERROR_CATEGORY_NAME_MANDATORY);
		final Map<Object, Object> av = new HashMap<>(otherData == null ? new HashMap<>() : otherData);
		...
		if (listIdParentCategory != null) {
			for (int i = 0; i < listIdParentCategory.size(); i++) {
				if (ObjectTools.safeIsEquals(listIdParentCategory.get(i), parentCategory.getIdCategory())) {
					final Map<? extends Serializable, ? extends Serializable> recordValues = er.getRecordValues(i);
					final Serializable idCategory = recordValues.remove(this.getColumnHelper().getCategoryIdColumn());
					final String categoryName = (String) recordValues.remove(this.getColumnHelper().getCategoryNameColumn());
